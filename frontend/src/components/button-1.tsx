...
export const Button1 = ({ children, className, variant = 'primary', disabled, ...props }: Button1Props) => {
  return (
    <button 
      className={cn(
        "!px-4 !py-2 !text-lg !font-medium !rounded-xl transition-colors focus:!outline-none !h-10 !flex !items-center !justify-center !gap-2 !w-fit [&>svg]:w-5 [&>svg]:h-5 [&>svg]:flex-shrink-0",
        disabled 
